...
- Sunlight through the windows with real shadows: a low afternoon sun traced back through each window's opening, glazing bars and blind, lighting the floor and the furniture it reaches, with the desks and chairs casting shadows from a shadow map drawn only when something changes. Shafts of sunlit dust hang in the beams on Quest 3 and desktop.
- Premium desktop overlay: frosted panels with the placards' gold rule, the controls as a list, pill buttons, and a warmer loading screen.
- Quest 2 keeps the reflections and the sun on the floor; picture-light pools, screen glazing, sun on the furniture and the sun shafts are switched off there (`src/xr/device.ts`). Busiest view: 39 draw calls on Quest 2, 45 on desktop.
